...
class randg(galg):
    ...
    def _calcCost(self):
        ...
        for i in range(1, 11):
            ...
            self.cost += self._calcEuclidDist(self.coords[0], self.coords[1])
            calc_dist_cnt += 1

            for j in range(1, len(self.coords) - 1):
        ...
        for i in range(len(self.coords)-10, len(self.coords) + 1):
            current_length = math.inf
            calc_dist_cnt = 0
            ...
            if(i < len(self.coords) - 1):
                ...
                self.coords[1] = self.coords[i]
                self.coords[i] = tmp3
            elif(i == len(self.coords)):
                tmp3 = self.coords[1]
                self.coords[1] = self.coords[i-1]
                self.coords[i-1] = tmp3
